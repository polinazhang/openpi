...
### A. `--metric="cosine"`: (for context only)
...
### B. `--metric="gradient"`: (for context only)
...
### C. `--metric="perturbance"`: (you need to implement this)
...
- Plan what adjustments need to be made.
- Implement mode `--metric="perturbance"`.
